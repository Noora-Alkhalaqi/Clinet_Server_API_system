...
class NewsClientGUI:
    ...
    def display_results(self, data_list, result_type):
        ...
        if not data_list:
            result_list.insert(1, "No data found.")
        else:
            for idx, entry in enumerate(data_list, start=1):
                if result_type == "Headlines":
                    if isinstance(entry, dict):
                        result_list.insert(idx, f"{idx}. Source: {entry.get('source', 'Unknown')} | Author: {entry.get('author', 'Unknown')} | Title: {entry.get('title', 'No Title')}")
                elif result_type == "Sources":
                    if isinstance(entry, dict):
                        result_list.insert(idx, f"{idx}. Source name: {entry.get('name', 'Unknown')}")
